...
vec2 wobble = 0.012 * sin(vec2(1.8, 1.4) * uTime + animUV.yx * 8.0);
vec2 grid = (animUV + wobble) * cellCount;

vec2 cell = fract(grid) - 0.5;
float thickness = mix(0.003, 0.018, eased);
...
float build = smoothstep(-0.15, 0.85, eased - edge);

// Fade in/out globally so the grid never becomes a full-white flash.
float softAppear = smoothstep(0.0, 0.08, eased);
float softExit = 1.0 - smoothstep(0.92, 1.0, eased);
float presence = softAppear * softExit;

float pulse = 0.5 + 0.5 * sin(uTime * 1.3 + length(grid) * 0.3);
float alpha = clamp(lines * build * pulse * presence, 0.0, 1.0);
